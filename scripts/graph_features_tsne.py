...
def main():
    ...
    windows_per_class = collect_windows_per_class(
        class_map, config['raw_data_dir'], config['window_size'],
    ...
    for cls in class_names:
        for i, seg in enumerate(windows_per_class[cls]):
            tasks.append((cls, i, seg, config['tau']))
    ...
    print(f"有效样本数: {len(y_G)}")
    print(f"原图特征维度: {X_G_scaled.shape[1]}, 线图特征维度: {X_LG_scaled.shape[1]}, 合并维度: {X_Both.shape[1]}")
    ...
    datasets = [
        (X_G_scaled, y_G, 'Original Graph Features'),
        (X_LG_scaled, y_LG, 'Line Graph Features'),
        (X_Both, y_G, 'Combined Features')
    ]
    ...
    plt.tight_layout()
    save_path = os.path.join(config['output_dir'], 'graph_features_tsne.png')
